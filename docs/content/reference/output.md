...
describes what users get. Pick a format with `-o`, or let urlscan choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
urlscan <command> -o table   # aligned columns for reading
urlscan <command> -o jsonl   # one JSON object per line, for piping
urlscan <command> -o json    # a single JSON array
urlscan <command> -o csv     # spreadsheet friendly
urlscan <command> -o tsv     # tab-separated
urlscan <command> -o url     # just the URL column
urlscan <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
urlscan <command> --fields id,title,url
```
...
```bash
urlscan <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
urlscan <command>            # a table, because this is a terminal
urlscan <command> | wc -l    # JSONL, because this is a pipe
```
