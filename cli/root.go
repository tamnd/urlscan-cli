// Package cli assembles the urlscan command tree from the urlscan
...
// NewApp assembles the kit application from the urlscan domain. The
...
//
// To add a command, declare it in urlscan/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := urlscan.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(urlscan.Domain{}).Register(app)
